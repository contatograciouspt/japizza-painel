...
const Dashboard = () => {
  ...
  const { data: bestSellerProductChart, loading: loadingBestSellerProduct, error } = useAsync(OrderServices.getBestSellerProductChart)
  const { data: dashboardRecentOrder, loading: loadingRecentOrder } = useAsync(() => OrderServices.getDashboardRecentOrder({ page: currentPage, limit: 8 }))
  ...
  const ordersPendentes = Array.isArray(allOrders) ? allOrders.filter(order => order.status === "Pendente").length : 0
  ...
  const yesterdayOrders = Array.isArray(allOrders) ? allOrders.filter(order => {
    const orderDate = new Date(order.createdAt)
    const today = new Date()
  ...
  const monthlyOrders = Array.isArray(allOrders) ? allOrders.filter(order => {
    const orderDate = new Date(order.createdAt)
  ...
  const todayTotalAmount = todayOrders.reduce((total, order) => total + (order.amount / 100), 0)
  const yesterdayTotalAmount = yesterdayOrders.reduce((total, order) => total + (order.amount / 100), 0)
  const monthlyTotal = monthlyOrders.reduce((total, order) => total + (order.amount / 100), 0)
